...
/**
 * Controlador para comunicación entre la vista y el modelo de la funcionalidad de Parametros de correo.
 * @author  ASSIS S.A.S
...
class ParametroCorreoController extends Controller
{
   ...
   public function store(Request $request)
   {
       ...
       try {
           ...
           if ($regCre) {
               ...
               return response(get_response_body(["Parametros correo, ha sido creado.", 2], $regCre), Response::HTTP_CREATED);
           }
           else {
               DB::rollback(); // Se devuelven los cambios, por que la transaccion falla
               return response(get_response_body(["Error al crear Parametros correo."]), Response::HTTP_CONFLICT);
           }
       }
       catch (Exception $e)
       {
   ...
   public function update(Request $request, $id)
   {
       ...
       try {
           ...
           if ($regMod) {
               DB::commit(); // Se cierra la transaccion correctamente
               return response(get_response_body(["Parametros correo, ha sido modificado.", 1], $regMod), Response::HTTP_OK);
           }
           else {
               DB::rollback(); // Se devuelven los cambios, por que la transaccion falla
               return response(get_response_body(["Error al modificar los Parametros correo."]), Response::HTTP_CONFLICT);;
           }
       }
       catch (Exception $e)
       {
   ...
   public function destroy($id)
   {
       ...
       try {
           ...
           if ($regEli){
               DB::commit(); // Se cierra la transaccion correctamente
               return response(get_response_body(["Parametros correo, ha sido eliminado.", 3]), Response::HTTP_OK);
           }
           else {
               DB::rollback(); // Se devuelven los cambios, por que la transaccion falla
               return response(get_response_body(["Error al eliminar los Parametros correo."]), Response::HTTP_CONFLICT);
           }
       }
       catch (Exception $e)
       {
