   ...
   */
   ...
   public function store(Request $request)
   {
      ...
      try { $datos = $request->all();
            ...
            $retVal = Validator::make( $datos, 
                                       ['persona_id'               => ['integer', 'required', Rule::exists('personas','id')->where(function ($query) { $query->where('personasEstadoRegistro', 1); }), ],
                                        'benefactor_id'            => ['integer', 'required', Rule::exists('donaciones','id')->where(function ($query) { $query->where('estado', 1); }), ],
                                        'donacionesFechaDonacion'  => 'date|nullable',
                                        'tipo_donacion_id'         => ['integer', 'required', Rule::exists('tipos_donacion','id')->where(function ($query) { $query->where('tipDonEstado', 1); }), ],
                                        'donacionesValorDonacion'  => '',                                       
                                        'donacionesEstadoDonacion' => 'string|required|max:2',
                                        'forma_pago_id'            => ['integer', 'required', Rule::exists('ciudades','id')->where(function ($query) { $query->where('ciudadesEstado', 1); }), ],
                                        'donacionesNumeroCheque'   => 'string|required|max:128',
                                        'banco_id'                 => ['integer', 'required', Rule::exists('bancos','id')->where(function ($query) { $query->where('bancosEstado', 1); }), ],
                                        'donacionesNumeroRecibo'   => 'string|required|max:128',
                                        'donacionesFechaRecibo'    => 'date|nullable',
                                        'donacionesNotas'          => 'string|required|max:512',
                                        'estado'                   => 'boolean|required',],
                                       $msgErr = [ 'persona_id.exists'       => 'El tipo de benefactor no existe o está en estado inactivo',
   ...
   public function show($id)
   {
      try { $datos['id'] = $id;
            ...
            $retVal = Validator::make($datos, ['id' => 'integer|required|exists:donaciones,id']);
            if ($retVal->fails())
               return response(get_response_body(format_messages_validator($retVal)), Response::HTTP_BAD_REQUEST);
      ...
      try { $datos = $request->all();
            ...
            $retVal = Validator::make( $datos, 
                                       ['id'                       => 'integer|required|exists:donaciones,id',
                                        ...
                                        'donacionesValorDonacion'  => '',
                                        'donacionesEstadoDonacion' => 'string|required|max:2',
                                        'forma_pago_id'            => ['integer', 'required', Rule::exists('formas_pago','id')->where(function ($query) { $query->where('forPagEstado', 1); }), ],
                                        'donacionesNumeroCheque'   => 'string|required|max:128',
                                        'banco_id'                 => ['integer', 'required', Rule::exists('bancos','id')->where(function ($query) { $query->where('bancosEstado', 1); }), ],
                                        'donacionesNumeroRecibo'   => 'string|required|max:128',
                                        'donacionesFechaRecibo'    => 'date|nullable',
