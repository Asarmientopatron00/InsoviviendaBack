...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::table('pagos', function (Blueprint $table) {
            $table->integer('pagosConsecutivo')->after('pagosDescripcionPago')->nullable();
        });

        Schema::table('pagos_detalle', function (Blueprint $table) {
            $table->decimal('pagDetSaldoCartera', $precision = 18, $scale = 2)->after('pagDetDiasMora')->nullable();
        });
        ...
        SP: BEGIN
            ...
            UPDATE PAGOS_DETALLE 
            SET PAGDETSALDOCARTERA = V_VALORSALDOCARTERA,
            UPDATED_AT = SYSDATE()
            WHERE PROYECTO_ID = P_NUMEROPROYECTO
            AND PAGDETNUMEROCUOTA = V_NUMEROCUOTA
            AND PAGDETFECHAPAGO = P_FECHAPAGO;
    ...
    {
        Schema::table('pagos', function (Blueprint $table) {
            $table->dropColumn('pagosConsecutivo');
        });

        Schema::table('pagos_detalle', function (Blueprint $table) {
            $table->dropColumn('pagDetSaldoCartera');
        });
    }
};
