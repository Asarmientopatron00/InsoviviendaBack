...
return new class extends Migration
{
    ...
    public function up()
    {
        $procedure4 = "DROP PROCEDURE IF EXISTS `SP_PlanAmortizacionGenerar`;
        ...
        SP: BEGIN
            ...
            DECLARE FECHA_VENCIMIENTO_SALDO_U DATE DEFAULT NULL;
            ...
            IF ID_PROYECTO_UNIFICADO <> 0 THEN
                SELECT COUNT(1)
                INTO CONTADOR
                FROM PLAN_AMORTIZACION_DEF
                WHERE PLAMDECUOTACANCELADA = 'S'
                AND PROYECTO_ID = ID_PROYECTO_UNIFICADO;
                
                IF CONTADOR > 0 THEN
                    SELECT PLAMDEFECHAVENCIMIENTOCUOTA
                    INTO FECHA_VENCIMIENTO_SALDO_U
                    FROM PLAN_AMORTIZACION_DEF
                    WHERE PLAMDECUOTACANCELADA = 'S'
                    AND PROYECTO_ID = ID_PROYECTO_UNIFICADO
                    ORDER BY PLAMDENUMEROCUOTA DESC
                    LIMIT 1;
                ELSE
                    SELECT DESEMBOLSOSFECHADESEMBOLSO 
                    INTO FECHA_VENCIMIENTO_SALDO_U
                    FROM DESEMBOLSOS 
                    WHERE PROYECTO_ID = ID_PROYECTO_UNIFICADO 
            ...
            IF ID_PROYECTO_UNIFICADO <> 0 THEN
                SET V_VALORCREDITO = V_VALORCREDITO + SALDO_UNIFICADO;
            END IF;
                ...
                IF V_FECHANORMALIZACION IS NOT NULL AND V_FECHANORMALIZACION > V_FECHAVENCIMIENTO THEN
                    BEGIN
                        ...
                        CURDESEMBOLSOSLOOP: LOOP
                            ...
                            IF CONTADOR = 1 AND SALDO_UNIFICADO <> 0 THEN
                                -- SET V_VALORDESEMBOLSO_PRO = V_VALORDESEMBOLSO_PRO + SALDO_UNIFICADO;
                                SELECT DATEDIFF(V_FECHANORMALIZACION, FECHA_VENCIMIENTO_SALDO_U) INTO DIAS_HASTA_FIN;
                                IF DIAS_HASTA_FIN < 0 THEN 
                                    SET DIAS_HASTA_FIN = 0;
                                ELSE 
                                    SELECT 30-DAY(FECHA_VENCIMIENTO_SALDO_U) INTO DIAS_HASTA_FIN;
                                    SELECT DAY(V_FECHANORMALIZACION) INTO DIAS_FIN;
                                    SELECT YEAR(V_FECHANORMALIZACION) - YEAR(FECHA_VENCIMIENTO_SALDO_U) - 1 INTO ANNOS_DIF;
                                    IF ANNOS_DIF >= 0 THEN
                                        SELECT MONTH(V_FECHANORMALIZACION) - MONTH(FECHA_VENCIMIENTO_SALDO_U) + 11 INTO MESES_DIF;
                                    ELSE 
                                        SELECT MONTH(V_FECHANORMALIZACION) - MONTH(FECHA_VENCIMIENTO_SALDO_U) - 1 INTO MESES_DIF;
                                    END IF;
                                    ...
                                    IF MESES_DIF < 0 THEN
                                        SET MESES_DIF = 0;
                                        IF ANNOS_DIF < 0 THEN
                                            SELECT DAY(FECHA_VENCIMIENTO_SALDO_U) INTO DIAS_HASTA_FIN;
                                            SET DIAS_FIN = DIAS_FIN - DIAS_HASTA_FIN;
                                            SET DIAS_HASTA_FIN = 0;
                                        END IF;
                                    END IF;
                                ...
                                SET V_VALORINTERESNORMALIZACION = ((((SALDO_UNIFICADO * V_TASANMV) / 100) / 30) * V_DIASNORMALIZACION);	
                            ...
                            IF MESES_DIF < 0 THEN
                                SET MESES_DIF = 0;
                                IF ANNOS_DIF < 0 THEN
                                    SELECT DAY(V_FECHADESEMBOLSO_PRO) INTO DIAS_HASTA_FIN;
                                    SET DIAS_FIN = DIAS_FIN - DIAS_HASTA_FIN;
                                    SET DIAS_HASTA_FIN = 0;
                                END IF;
                            END IF;
            ...
            ELSE
                IF V_FECHANORMALIZACION IS NOT NULL AND V_FECHANORMALIZACION > V_FECHAVENCIMIENTO THEN
                    ...
                    IF MESES_DIF < 0 THEN
                        SET MESES_DIF = 0;
                        IF ANNOS_DIF < 0 THEN
                            SELECT DAY(FECHA_VECIMIENTO) INTO DIAS_HASTA_FIN;
                            SET DIAS_FIN = DIAS_FIN - DIAS_HASTA_FIN;
                            SET DIAS_HASTA_FIN = 0;
                        END IF;
                    END IF;
